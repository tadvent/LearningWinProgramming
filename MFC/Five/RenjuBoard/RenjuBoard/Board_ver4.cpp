...
int CBoard::SearchVCF(const char color, const bool clean){
    //检查color方有无VCF，若有返回1，否则返回0；VCF序列由vcfList链表返回
    int result = 0;
    vcfList.clear();

    result=DoSearchVCF(color);

    if(result)if(clean)DoCleanVCF(color);
    return result;
}
    ...
    if(HaveFour(-color,x,y)){   //对方有四
        ...
        if(4 == status){    //己方冲四
            ...
            if(color == WHITE && IsFoul(defx,defy)){    //己方是白棋并且这步冲四逼黑棋禁手
                ...
                vcfList.push_back(FourRush(CPoint(x,y),CPoint(defx,defy)));
                TryMove(-color,defx,defy);
                result=DoSearchVCF(color);
                TryMove(EMPTY,defx,defy);
