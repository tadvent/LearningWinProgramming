...
public:
    ...
    FourRush(const CPoint &RushPt, const CPoint &DefensePt):rush(RushPt),defense(DefensePt){}
};
...
public:
    ...
    deque<list<FourRush> > vcfStateQu;
    list<FourRush> vcfList;
    UINT nCurrent;
...
/*    //有活三或VCF返回1，否则返回0，VCF走棋顺序及挡点由VCF向量返回(阻挡对方时用)
    ...
    int SearchVCF(const char color/*, list<FourRush> &vcfList*/);
