...
int CBoard::DoSearchVCF(const char color/*, list<FourRush> &vcfList*/){
    ...
    for(vcfTreeNode *p=pCurrent;p->pParent!=NULL;p=p->pParent){
        TryMove(color,(p->frRush).rush.x,(p->frRush).rush.y);
        TryMove(-color,(p->frRush).defense.x,(p->frRush).defense.y);
    }
    ...
    if(HaveFour(-color,x,y)){   //对方有四
        ...
        if(4 == status){    //己方冲四
            ...
            if(color == WHITE && IsFoul(defx,defy)){    //己方是白棋并且这步冲四逼黑棋禁手
                ...
                p->frRush=FourRush(CPoint(x,y),CPoint(defx,defy));
                p->pParent=pCurrent;
    ...
    //撤局
    for(vcfTreeNode *p=pCurrent;p->pParent!=NULL;p=p->pParent){
        TryMove(EMPTY,(p->frRush).rush.x,(p->frRush).rush.y);
        TryMove(EMPTY,(p->frRush).defense.x,(p->frRush).defense.y);
    }
    return result;
}