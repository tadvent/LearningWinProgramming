...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lmCmdLine,int nShowCmd){
    static wchar_t szAppName[] = L"Colors1";
    HWND hwnd;
    MSG msg;
    WNDCLASS wndclass;
    ...
    if(!RegisterClass(&wndclass)){
        MessageBox(NULL,L"Requires Windows NT!",szAppName,MB_ICONERROR);
    }

    hwnd = CreateWindowW(szAppName,L"Color Scroll",WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT message, WPARAM wParam, LPARAM lParam){
    ...
    switch(message){
        case WM_CREATE:
            ...
            hwndRect = CreateWindow(L"static",NULL,WS_CHILD|WS_VISIBLE|SS_WHITERECT,
                0,0,0,0,hwnd,(HMENU)9,hInstance,NULL);

            for(i=0;i<3;++i){
                //The three scroll bars have IDs 0,1, and 2, with
                //scroll bar ranges from 0 through 255.
                hwndScroll[i] = CreateWindowW(L"scrollbar",NULL,
                ...
                hwndLabel[i] = CreateWindow(L"static",szColorLabel[i],
                    WS_CHILD|WS_VISIBLE|SS_CENTER,0,0,0,0,
                    hwnd,(HMENU)(i+3),hInstance,NULL);
                //The three color-value text fields have IDs 6,7,
                //and 8, and initial text strings of "0".
                hwndValue[i] = CreateWindow(L"static",L"0",
                    WS_CHILD|WS_VISIBLE|SS_CENTER,0,0,0,0,
